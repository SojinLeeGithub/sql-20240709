...
-- 1. 모든 컬럼에 대하여 삽입
-- INSERT INTO 테이블명 VALUES(데example_tablecolumn1example_colum데이터1, 데이터2, ...);
-- 테이블의 구조의 컬럼 순서에 맞게 모든 데이터를 입력해야 함
...
-- SELECT : 테이블에서 레코드를 조회할 때 사용하는 명령 (
...
SELECT colum FROM example_table;

-- 3. 특정 레코드 선택 조회
-- SELECT 조회 할 컬럼 FROM 테이블명 WHERE 조건;
...
-- DELETE FROM 테이블명
 ...
 -- INSERT INTO SELECT : 삽입 작업시에 조회 결과를 사용한
