...
INSERT INTO not_null_table (not_null_column) VALUES(2,1);
...
CREATE TABLE composite_table (
    ...
    -- FORGIN KEY 조건 : 특정 커럼을 다른 테이블 혹은 같은 테이블의 기본키 컬럼과 연결하는 제약
    -- FOREIGN KEY 제약조건을 특정 컬럼에 적용할 떄는 반드시 데이터 타입이 참조하고자 하는 컬럼의 타입과 일치해야함
...
-- FOREIGNKEY 제약조건으로 참조되어지고 있는 테이블의 컬럼은 수정, 삭제가 불가능
...
-- CASCADE : 참조하고 있는 테이블의 데이터가 삭제 또는 수정 된다면,
--            참조하고 있는 테이블에서도 삭제 또는 수정이 같이 일어남

-- SET NULL : 참조되고 있는 테이블의 데이터가 삭제 또는 수정 된다면
...
CREATE TABLE optional_foreign_table (
	primary_column INT PRIMARY KEY,
	foreignz_column INT,
    FOREIGN KEY (foreign_column) REFERENCES key_table (primary_Key
...
-- CHECK 제약조건 : 해당 컬럼에 값을 제한하는 제약alter
CREATE TABLE check_table (
	primary_column INT PRIMARY KEY,
    check_column VARCHAR(5) CHECK(check_column )
);
...
CREATE TABLE defalt_table (
	primary_colum INT PRIMARY KEY AUTO_INCREMENT,
    column1 INT,
